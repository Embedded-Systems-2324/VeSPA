...
assign readReg = m_axi_rdata[9:2];

//          m_axi_rdata
// | 31 : 10  | 9 : 2 |   1    |   0    | 
//| reserved | rxReg | rxDone | txDone |

//          m_axi_wdata
// |    31    | 30 : 11  | 10 : 3 |   2   |  1   |  0   | 
//| reserved | baudrate | txReg | uartEn | rxEn | txEn |
...
always@(posedge clk) begin
    ...
    if(currentBaud != baudrate) begin // Check and configure the baudrate value if changed
        case(baudState)
            stopUart: begin
                m_axi_wdata <= {1'b0, currentBaud, txReg, 1'b0, 1'b0, 1'b0};
                baudState <= confBaud;
            end
            confBaud: begin
                m_axi_wdata <= {1'b0, baudrate, txReg, 1'b0, 1'b0, 1'b0};
                baudState <= enableUart;
            end
            enableUart: begin
                m_axi_wdata <= {1'b0, baudrate, txReg, uartEn, rxEn, 1'b0};
    ...
    if(uartState != uartEn) begin   // Enable or disable uartState reg according to uartEn reg
        m_axi_wdata <= {1'b0, baudrate, txReg, uartEn, rxEn, 1'b0};
        uartState <= uartEn;
    end
        
    else if (rxDone & uartEn) begin // Store the last rx value to be sent later
        rxReg <= readReg;
    end

    else if ((txStart | txProc) & uartEn) begin // Transmit and wait until the character is sent
        case(txState)
            WriteTxData: begin
                m_axi_wdata <= {1'b0, baudrate, txReg, uartEn, rxEn, 1'b0};                         
                txState <= SendTxData;
            ...
            SendTxData: begin
                m_axi_wdata <= {1'b0, baudrate, txReg, uartEn, rxEn, 1'b1};     
                txState <= WaitTxData;
            end 
            WaitTxData: begin
                if (txDone) begin
