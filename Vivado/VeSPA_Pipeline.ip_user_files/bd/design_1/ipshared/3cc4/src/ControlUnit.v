...
assign o_PcSel = (r_CurrentState == ST_RUN) ? ((i_IntRequest) ? `PC_SEL_INT :
                                               (i_OpCode == `OP_BXX) ? `PC_SEL_BXX :
                                               (i_OpCode == `OP_JMP) ? `PC_SEL_JMP :
                                               (i_OpCode == `OP_RET) ? `PC_SEL_RET :
                                               (i_OpCode == `OP_RETI && !i_IntPending) ? `PC_SEL_RETI: 
                                               (i_OpCode == `OP_RETI &&  i_IntPending) ? `PC_SEL_INT : `PC_SEL_ADD4) : `PC_SEL_ADD4;
...
assign o_WrEnRf = (i_OpCode == `OP_ADD || i_OpCode == `OP_SUB || i_OpCode == `OP_OR  ||
                  ...
                  (i_OpCode == `OP_JMP && i_ImmOp == 1'b1)) ? 1'b1 : 
                   i_OpCode == `OP_RR || i_OpCode == `OP_RL || 
...
assign o_MemAddrSel = (i_OpCode == `OP_STX || i_OpCode == `OP_LDX) ? `MEM_SEL_OPX : `MEM_SEL_IMM ;
                   
assign o_AluCtrl =  (i_OpCode == `OP_ADD) ? 5'b00001 :
                    (i_OpCode == `OP_SUB || i_OpCode == `OP_CMP) ? 5'b00010 :
                    (i_OpCode == `OP_OR)  ? 5'b00011 : 
                    (i_OpCode == `OP_AND) ? 5'b00100 :
                    (i_OpCode == `OP_NOT) ? 5'b00101 : 
                    (i_OpCode == `OP_XOR) ? 5'b00110 : 
                    (i_OpCode == `OP_RR) ? 5'b01111 : 
                    (i_OpCode == `OP_RL) ? 5'b10000 : 5'b00000;

assign o_AluOp2Sel = (i_ImmOp == 1'b1) ? 1'b1 : 1'b0;

assign o_AluEn = (i_OpCode == `OP_ADD || i_OpCode == `OP_SUB || i_OpCode == `OP_OR  ||
                  i_OpCode == `OP_AND || i_OpCode == `OP_NOT || i_OpCode == `OP_XOR ||
                  i_OpCode == `OP_CMP || i_OpCode == `OP_RR || i_OpCode == `OP_RL ) ? 1'b1 : 1'b0;
